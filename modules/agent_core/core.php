...
final class core extends Factory
{
    ...
    public function getSystemDefault(bool $in_sandbox = true): array
    {
        ...
        $prompts[] = '**读**:缺信息顺序 ram→daily→important(system已加载)。消息≤2条只读daily。提时间读对应daily(可ram)。提人/事/偏好无上下文先important再daily(可ram)。ram随意读写，禁编造/重复读。';
        ...
        $prompts[] = '## 任务执行规则';
        $prompts[] = '- 禁止中途停止，必须连续执行至完成。';
        $prompts[] = '- 每步前输出{简述}，之后继续，禁输出“已完成”等结束词。工具失败重试≤2次，仍失败报用户。';
        $prompts[] = '- 完成标志：全部达成后输出“所有任务执行完毕，结果如下：”并附成果。';
