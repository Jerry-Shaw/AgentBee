...
class utils extends Factory
{
            ...
            try {
                ...
                if ([] !== $tool_names) {
                    $metadata = array_filter(
                        ...
                        {
                            return in_array($item['name'] ?? $item['function']['name'], $tool_names, true);
                        }
                    );
                }
                ...
                foreach ($metadata as $index => $item) {
                    if (isset($item['name'])) {
                        $metadata[$index]['name'] = $json_data['name'] . '-' . $item['name'];
                    } else {
    ...
    public function getMainPrompt(): string
    {
        ...
        $prompts[] = '- **如实直接**：确定直说，不确定先查，查不到则明说。必要时可询问用户，优先自查。禁猜测、禁编造、禁套话。只答所问，不主动扩展。';
        ...
        $prompts[] = '- **读取**：新会话先读10条misc建立上下文；不足时按offset每次追加5条（上限3次）；需某天完整对话流时按date读misc（原文）或daily（总结），当日为空则向前回溯1-2天。同参数禁止重调。';
        $prompts[] = '- **搜索**：上下文未覆盖用户问题时搜索全部层级，关键词取1-5个最具辨识度的词（避免泛化词），多词同时命中用and、任一即可用or；搜完即止，换词重试3次仍无记录则告知"记忆中没有相关记录"。命中后如需补全完整上下文，按命中记录的日期读取当日misc/daily。';
    ...
    public function getChildPrompt(string $worker_name, string $worker_role): string
    {
        ...
        $prompts[] = '- **如实直接**：确定直说，不确定则明说，必要时可询问用户。禁猜测、禁编造、禁套话。只答所问，不主动扩展。';
        $prompts[] = '- **规划验证**：复杂/重要任务先评估方案与风险。匹配技能必须调`System-loadSkill`加载，并严格遵循完整指令，禁止使用通用工具或自拟流程替代；无匹配技能则使用适当工具。';
