...
class tools
{
    public const META = [
        [
            ...
            'function' => [
                'name' => 'exec',
                'description' => '执行系统命令。限制:program必须是独立可执行文件(如powershell.exe,git,python,ls)。禁止cmd内置:dir,echo,type,cd,mkdir,copy,del,move,ren,rd。argv必须为字符串数组(如["-l","-a"]),禁止单字符串。timeout(秒,默认300),work_path(可选)。返回 {"output":"","error":""}',
                'parameters' => ['type' => 'object', 'properties' => ['program' => ['type' => 'string'], 'argv' => ['type' => 'array', 'items' => ['type' => 'string']], 'timeout' => ['type' => 'integer', 'default' => 300], 'work_path' => ['type' => 'string']], 'required' => ['program', 'argv']],
            ],
        ],
        [
            'type' => 'function',
